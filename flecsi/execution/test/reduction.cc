...
void driver(int argc, char ** argv) {
  ...
  for(int cycle=1; cycle < 10; cycle++) {
    ...
    Legion::Future global_max_future =
      flecsi::execution::context_t::instance().reduce_max(local_future);
    const bool silence_warnings = true;
    double global_max = global_max_future.get_result<double>(silence_warnings);

    Legion::Future global_min_future =
      flecsi::execution::context_t::instance().reduce_min(local_future);
    double global_min = global_min_future.get_result<double>(silence_warnings);
